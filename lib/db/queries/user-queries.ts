...
export async function createUser(
  ...
  try {
    return await db.transaction(async (tx) => {
      ...
      // Every user gets a 1:1 Balance row. Plan starts null — users must
      // subscribe (or be assigned to a plan) to get access.
      await tx.insert(balance).values({
        userId: newUser.id,
        plan: "free",
        tokens: FREE_TIER_ENTITLEMENTS.tier_1.tokenBonus,
        imageGenerations: FREE_TIER_ENTITLEMENTS.tier_1.imageBonus,
        videoGenerations: FREE_TIER_ENTITLEMENTS.tier_1.videoBonus,
        webSearches: FREE_TIER_ENTITLEMENTS.tier_1.searchQuota,
        searchDepth: FREE_TIER_ENTITLEMENTS.tier_1.searchDepthAllowed,
      });
